...
export function useChat(conversationId: string | undefined) {
  ...
  useEffect(() => {
    ...
    apiClient.get(`/conversations/${conversationId}/messages`).then((res) => setMessages(res.data.messages));
  }, [conversationId]);
  ...
  const sendMessage = useCallback(
    async (content: string, currentConversationId?: string) => {
      setIsSending(true);
      setStreamingContent('');
      const optimisticMessage: Message = { id: `temp-${Date.now()}`, conversationId: currentConversationId ?? '', role: 'USER', content, createdAt: new Date().toISOString() };
