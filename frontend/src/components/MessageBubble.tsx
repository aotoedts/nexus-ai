...
export function MessageBubble({ message }: Props) {
  ...
  return (
    <div className={`relative flex gap-3 px-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`max-w-[75%] rounded-2xl px-4 py-3 text-[15px] leading-relaxed ${isUser ? 'bg-nexus-600 text-white rounded-tr-sm' : 'bg-ink-800 text-gray-100 rounded-tl-sm border border-ink-700'}`}>
