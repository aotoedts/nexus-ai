...
export function InstallAppBanner() {
  ...
  const dismiss = () => { sessionStorage.setItem('nexus_install_dismissed', '1'); setDismissed(true); };
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-ink-800 bg-ink-900 px-4 py-2.5 text-sm">
      ...
      <div className="flex items-center gap-3">
        <button onClick={async () => { await installEvent.prompt(); setInstallEvent(null); }} className="rounded-md bg-nexus-600 px-3 py-1 text-xs font-medium text-white hover:bg-nexus-500">
          Instalar
        </button>
        <button onClick={dismiss} className="text-gray-500 hover:text-gray-300"><X size={15} /></button>
      </div>
    </div>
  );
}
