...
- **L — Liskov Substitution**: qualquer implementacao de `IModelAdapter`
  (`QwenAdapter`, `MockAdapter`, `OpenAICompatibleAdapter`, `OllamaAdapter`)
  e intercambiavel; o restante do sistema so conhece a interface.
...
lugar, `ModelAdapterFactory.ts`, com base na variavel `AI_PROVIDER`
(`qwen` | `openai_compatible` | `ollama` | `mock`). Isso significa que
trocar de modelo (Qwen via API, Qwen local via Ollama, ou qualquer outro
modelo compativel) exige apenas:
